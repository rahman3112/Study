...
const Todo = () => {
  ...
  useEffect(() => {
    const fetchTasks = async () => {
      try {
        const response = await axios.get("https://study-backend-k311.onrender.com/api/tasks");
  ...
  const handleAddTask = async () => {
    ...
    try {
      const response = await axios.post("https://study-backend-k311.onrender.com/api/tasks", newTask);
  ...
  const handleTaskChange = async (id, updates) => {
    try {
      const response = await axios.put(`https://study-backend-k311.onrender.com/api/tasks/${id}`, updates);
  ...
  const handleConfirmDelete = async () => {
    try {
      await Promise.all(selectedTasks.map((id) => axios.delete(`https://study-backend-k311.onrender.com/api/tasks/${id}`)));
